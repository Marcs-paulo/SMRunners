...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function animateScroll() {
    ...
    const antecipacao = 600;
    const percentVisible = 1 - Math.min(Math.max((rect.top - antecipacao) / windowHeight, 0), 1);
...
document.addEventListener('click', (event) => {
  const clicouForaMenu = !menuMobile.contains(event.target);
  const clicouForaBtn = !hamburguer.contains(event.target);

  if (menuMobile.classList.contains('ativo') && clicouForaMenu && clicouForaBtn) {
